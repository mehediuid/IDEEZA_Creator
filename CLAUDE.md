...
- **Persistence:** the *document* (`objects, pcbBoard, twoD, threeD, gridSize, unit, snapEnabled, designRules, pcbDrcConfig, pcbLayers, pcbNets, pcbDefaults, boardSettings`) auto-saves (debounced) to `localStorage` under `ideeza:pcb:doc:<projectId>`, scoped by `ideeza:manual:active`. So layer colours/visibility/lock, net colours, place defaults and DRC rule-tuning now survive reload. UI flags (zoom, panels, menus, sheets) are session-only. `saveDoc()` force-writes; `sanitizePcbDoc()` shape-checks every key on load.
...
- **Left tool palette** (split-button flyouts): Select (Pointer/Lasso/Area/Hand) · Wire (Wire/Bus/Bus-entry) · **Shapes** (Rectangle/Line/Polyline/Circle/Ellipse/Arc/Bezier) · Net Label · Power · No Connect · Junction · Text · Eraser. Every option arms a real place/draft tool. **Lasso** = real freeform-polygon selection (ray-cast point-in-polygon, `state.lasso` + start/update/commit/cancel), **Area select** = real marquee (always rubber-bands on drag).
...
- **Left tool palette** (shared `ToolPalette`, same split-button flyout UI as the schematic) — PCB **modal-tool** set (all draw/place/route tools live here, not the top toolbar): Select (Pointer/Lasso/Area/Hand) · Route (Track/Diff-pair/Length-Tuning) · Pad (Pad/Test-Point/Shaped-Pad) · Via (Via/Suture/Via-Fence) · Region (Copper/Fill/Slot/Prohibited/Board-outline/Constraint/FPC-Stiffener) · Line (Line/Polyline) · Dimension · Text · Eraser. Every option arms a real PCB tool. *(The never-implemented Stretch-Track + Routing-Corner tools were removed from the Route flyout + Route menu — they armed a tool id with no handler.)*
...
- **Exports generate real files** (`exporters.ts`): **Gerber** (server kicad-cli → zip) · **BOM** (CSV) · **Pick&Place** (CSV/TXT/JSON) · **DXF** (R12) · **SVG** · **PDF** (hand-rolled vector) · **PNG** (2D SVG raster; 3D via `export3dPng` canvas capture) · **3D STL/OBJ**. *(STEP intentionally omitted — needs a CAD kernel; no stub. The **Delete Objects** dialog Confirm now really deletes per checked category — `deleteObjectsByCategory` + `del-objects.ts` `delCategoryOf`.)*
