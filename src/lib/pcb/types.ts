...
export type BottomTab = "logs" | "device" | "drc" | "find" | "prop";

// A single Find Result row (populated by the Ctrl+F Find & Replace "Find All").
export interface FindMatch {
  id: string;          // canvas object id (for select-on-click)
  objectId: string;    // display id, e.g. $1I10
  page: string;
  device: string;
  symbol: string;
  name: string;
  globalNet: string;
  pinName: string;
  kind: string;        // drives the object-type filter chips
}
export type MenuId =
  | "edit"
  | "view"
...
export interface CanvasObject {
  ...
  // Editing context: "schematic" objects render only in Schematic mode,
  // "pcb" only in PCB/2D. Undefined = shown in every mode (back-compat).
  scope?: "schematic" | "pcb";
  // Multi-sheet: which schematic sheet this object lives on (undefined = the
  // first sheet, for back-compat with pre-multi-sheet documents).
  sheetId?: string;
  // Cross-probe link: for a PCB footprint generated by Convert, the id of the
  // schematic symbol it came from (so Cross Probe can jump between them).
  sourceId?: string;
  // Real polygon geometry (rings, LOCAL coords relative to x,y) — produced by a
  // boolean Combine op. Ring[0] is the outer contour; extra rings are holes.
  points?: { x: number; y: number }[][];
}

// ERC severity — the 5-level scale from the EasyEDA-Pro ERC/DRC spec.
// ("ignore" rules are simply never emitted, so findings use the four visible
// levels.)
export type ErcSeverity = "note" | "warning" | "error" | "fatal";

// One Electrical Rule Check finding (computed live from schematic connectivity).
...
// Right-click "Filter ▸" — restricts the selectable set to ONE category ("Only
// X"). Category id → predicate over object kind. "all"/"common" means no
// restriction. The active category is stashed in boardSettings under
// selFilterSchematic / selFilterPcb so both select paths honour it for free.
const SCHEM_SYMBOL_KINDS = new Set<string>([
...
export const SEL_FILTER_KINDS: Record<string, (kind: string) => boolean> = {
  // schematic
  pin: (k) => k === "pin",
  symbol: (k) => SCHEM_SYMBOL_KINDS.has(k),
  wirebus: (k) => k === "wire" || k === "bus",
  pinpair: (k) => k === "netTie" || k === "pin",
  net: (k) => k === "net" || k === "netLabel" || k === "netFlag" || k === "globalLabel" || k === "hierLabel" || k === "netBusLabel",
  // pcb
  track: (k) => k === "track" || k === "diffPair",
  padvia: (k) => k === "pad" || k === "via" || k === "sutureVias",
  copper: (k) => k === "polygon" || k === "fillRegion",
};

// `mode` gates the PCB Selection Filter (a PCB-only key filter). On top of that,
// the right-click "Only X" category filter applies in BOTH modes: if a category
// is active and this kind isn't in it, the object is unselectable.
export function isSelectable(kind: string, bag: Record<string, unknown>, mode?: string): boolean {
  const cat = (mode === "schematic" ? bag.selFilterSchematic : bag.selFilterPcb) as string | undefined;
  if (cat && cat !== "all") {
...
export interface PcbState {
  ...
  // PCB "Flip Board" — mirror the view to inspect the bottom of the board.
  boardFlipped: boolean;
  // Net highlight (right-click "Highlight Net" / "Unhighlight All"): objects
  // whose `net` matches glow; null = nothing highlighted.
  highlightedNet: string | null;
  // Object ids on the highlighted net (computed from live connectivity, since
  // schematic objects don't carry a stored `net`).
...
export const DEFAULT_SCHEM_TITLE_FIELDS: Array<{ key: string; label: string; on: boolean; valueOn: boolean; value: string }> = [
  { key: "title", label: "Title", on: true, valueOn: true, value: "" },
  { key: "docNo", label: "Doc. No.", on: true, valueOn: true, value: "SB-001" },
  { key: "revision", label: "Revision", on: true, valueOn: true, value: "C" },
  { key: "date", label: "Date", on: true, valueOn: true, value: "2026-07-16" },
...
// A realistic, fully-editable MCU circuit seeded onto the schematic sheet so
// every symbol type is present (IC, R/C/L, diode/LED, crystal, power rails,
// net labels, junctions, no-connect, pins) wired together. Rendered by
// <PlacedObjects/>, so each element is select / move / delete / rotate-able and
// shows in the Properties inspector. Objects are scope:"schematic" so they only
// appear in Schematic mode (never leak into PCB/2D).
function so(kind: string, x: number, y: number, extra: Partial<CanvasObject> = {}): CanvasObject {
  return { id: `sch-${kind}-${x}-${y}`, kind, x, y, scope: "schematic", ...extra };
}
function sw(x: number, y: number, endX: number, endY: number): CanvasObject {
  return { id: `sch-w-${x}-${y}-${endX}-${endY}`, kind: "wire", x, y, endX, endY, scope: "schematic" };
}
export const DEFAULT_SCHEM_OBJECTS: CanvasObject[] = [
  // ── Front-end: Vbus · I_LOAD · Rshunt · Rload → GND ──
  so("text", 112, 116, { text: "Vbus" }),
  so("currentSource", 140, 180, { text: "I_LOAD" }),
  sw(140, 160, 140, 132),
  so("resistorBox", 140, 300, { text: "Rshunt", rotation: 90 }),
  sw(140, 200, 140, 276),
  so("junction", 140, 276),
  so("resistorBox", 140, 420, { text: "Rload", rotation: 90 }),
  sw(140, 324, 140, 396),
  so("junction", 140, 324),
  so("gnd", 140, 472),
  sw(140, 444, 140, 464),
  // ── Differential input: R1/R2 + C2/C1/C3 filter + Rgain ──
  so("resistorBox", 250, 276, { text: "R1" }),
  sw(140, 276, 226, 276),
  sw(274, 276, 485, 276),
  so("resistorBox", 250, 324, { text: "R2" }),
  sw(140, 324, 226, 324),
  sw(274, 324, 485, 324),
  so("junction", 340, 276),
  so("junction", 340, 324),
  so("capacitor", 340, 300, { text: "C1", rotation: 90 }),
  sw(340, 276, 340, 282),
  sw(340, 318, 340, 324),
  so("junction", 300, 276),
  so("capacitor", 300, 232, { text: "C2", rotation: 90 }),
  sw(300, 250, 300, 276),
  so("gnd", 300, 196),
  sw(300, 214, 300, 188),
  so("junction", 300, 324),
  so("capacitor", 300, 392, { text: "C3", rotation: 90 }),
  sw(300, 324, 300, 374),
  so("gnd", 300, 436),
  sw(300, 410, 300, 428),
  so("resistorBox", 430, 300, { text: "Rgain", rotation: 90 }),
  so("junction", 430, 276),
  so("junction", 430, 324),
  // ── U1 instrumentation amp ──
  so("opamp", 505, 300, { text: "U1" }),
  sw(485, 276, 485, 292),
  sw(485, 324, 485, 308),
  // ── U1 supply decoupling: +5V · C6 · GND ──
  so("vcc5v", 470, 175),
  so("capacitor", 470, 222, { text: "C6", rotation: 90 }),
  sw(470, 181, 470, 204),
  so("gnd", 470, 262),
  sw(470, 240, 470, 254),
  // ── Rail decoupling: +5V · C8 · C9 · GND ──
  so("vcc5v", 700, 150),
  sw(700, 156, 700, 180),
  sw(650, 180, 750, 180),
  so("junction", 700, 180),
  so("junction", 680, 180),
  so("junction", 740, 180),
  so("capacitor", 680, 205, { text: "C8", rotation: 90 }),
  sw(680, 223, 680, 243),
  so("gnd", 680, 251),
  so("capacitor", 740, 205, { text: "C9", rotation: 90 }),
  sw(740, 223, 740, 243),
  so("gnd", 740, 251),
  // ── Output: U1 → R3 → Vout (C4 to GND) ──
  sw(528, 300, 560, 300),
  so("junction", 560, 300),
  so("resistorBox", 600, 300, { text: "R3" }),
  sw(560, 300, 576, 300),
  sw(624, 300, 700, 300),
  so("text", 712, 294, { text: "Vout" }),
  so("junction", 670, 300),
  so("capacitor", 670, 350, { text: "C4", rotation: 90 }),
  sw(670, 300, 670, 332),
  so("gnd", 670, 398),
  sw(670, 368, 670, 390),
  // ── Reference stage: R4 · C5 → U2 (Vref) · C7 → U1 ref ──
  so("opamp", 690, 478, { text: "U2" }),
  so("text", 760, 464, { text: "Vref" }),
  sw(670, 470, 755, 470),
  so("resistorBox", 600, 486, { text: "R4" }),
  sw(624, 486, 670, 486),
  sw(560, 300, 560, 486),
  so("junction", 560, 486),
  sw(560, 486, 576, 486),
  so("capacitor", 560, 530, { text: "C5", rotation: 90 }),
  sw(560, 486, 560, 512),
  so("gnd", 560, 578),
  sw(560, 548, 560, 570),
  so("vcc5v", 770, 500),
  so("capacitor", 770, 539, { text: "C7", rotation: 90 }),
  sw(770, 506, 770, 521),
  so("gnd", 770, 583),
  sw(770, 557, 770, 575),
  sw(713, 478, 505, 478),
...
// A tiny demo the schematic sheet ships with so a new board is never blank — a
// simple RC low-pass: Vin → R1 → node(Vout) → C1 → GND. The full current-sense
// amp still loads on demand via File ▸ "Load Sample Circuit".
export const DEMO_SCHEM_OBJECTS: CanvasObject[] = [
  so("text", 150, 214, { text: "Vin" }),
  so("resistorBox", 280, 240, { text: "R1" }),
  sw(176, 240, 256, 240),
  sw(304, 240, 400, 240),
  so("junction", 400, 240),
  so("text", 470, 234, { text: "Vout" }),
  sw(400, 240, 456, 240),
  so("capacitor", 400, 300, { text: "C1", rotation: 90 }),
  sw(400, 240, 400, 282),
  so("gnd", 400, 362),
  sw(400, 318, 400, 354),
];
...
export const initialState: PcbState = {
  ...
  compFlipH: false,
  compZ: "front",
  // New boards ship with a small demo (RC low-pass) so the sheet is never
  // blank; the full current-sense-amp sample loads on demand via File ▸
  // "Load Sample Circuit" (actions.loadSampleSchematic).
  objects: DEMO_SCHEM_OBJECTS,
  selectedIds: [],
  clipboardObjects: [],
  draftWire: null,
  rubberBand: null,
