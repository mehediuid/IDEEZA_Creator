...
type Command = { label: string; icon?: string; onClick: () => void };
...
// ⌘K command palette — searches every menu action (breadcrumb-labelled).
// Self-contained: arrow-key nav, Enter to run, Esc / backdrop to close. Mounted
// fresh each open (parent gates on `paletteOpen`), so initial state IS the
// reset — no reset-in-effect.
function CommandPalette({ commands, onClose }: { commands: Command[]; onClose: () => void }) {
  ...
  return (
    <div
      ...
      aria-label="Search commands"
      onClick={onClose}
      style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "0 16px", paddingTop: "14vh" }}
    >
      <div aria-hidden style={{ position: "absolute", inset: 0, background: "color-mix(in srgb, var(--color-bg-page) 55%, transparent)", backdropFilter: "blur(3px)" }} />
      <div
        onClick={(e) => e.stopPropagation()}
        style={{ position: "relative", width: "100%", maxWidth: 600, overflow: "hidden", borderRadius: "var(--radius-2xl, 16px)", border: "var(--border-width-1) solid var(--color-border-default)", background: "var(--color-bg-surface)", boxShadow: "0 24px 64px -16px rgba(0,0,0,.4)", animation: "ideeza-pop .16s cubic-bezier(.2,.9,.3,1.2)" }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: "var(--spacing-4)", height: 54, padding: "0 var(--spacing-7)", borderBottom: "var(--border-width-1) solid var(--color-border-subtle)" }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--color-text-tertiary)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
            <circle cx="11" cy="11" r="7" />
            <path d="M21 21l-4.3-4.3" />
          </svg>
          <input
            ...
            style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontSize: "var(--font-size-md)", color: "var(--color-text-primary)", fontFamily: "inherit" }}
          />
          <kbd style={{ fontSize: 11, fontWeight: 600, color: "var(--color-text-tertiary)", border: "var(--border-width-1) solid var(--color-border-default)", borderRadius: "var(--radius-sm)", padding: "2px 6px", fontFamily: "var(--font-family-mono), monospace" }}>
            ESC
          </kbd>
        </div>
        <div ref={listRef} role="listbox" aria-label="Commands" style={{ maxHeight: 420, overflowY: "auto", padding: "var(--spacing-3)" }}>
          {filtered.length === 0 ? (
            <div style={{ padding: "40px 20px", textAlign: "center", fontSize: "var(--font-size-md)", color: "var(--color-text-tertiary)" }}>
              No actions for “{query}”
            </div>
          ) : (
            filtered.map((cmd, idx) => {
              ...
              return (
                <button
                  ...
                  style={{ display: "flex", alignItems: "center", gap: "var(--spacing-5)", width: "100%", height: 44, padding: "0 var(--spacing-5)", borderRadius: "var(--radius-md)", border: "none", cursor: "pointer", textAlign: "left", background: active ? "var(--color-bg-brand-subtle)" : "transparent", color: active ? "var(--color-text-brand)" : "var(--color-text-primary)" }}
                >
                  <span style={{ width: 18, height: 18, flex: "0 0 auto", display: "inline-flex", color: active ? "var(--color-violet-600)" : "var(--color-text-tertiary)" }}>
                    <DsIcon name={cmd.icon} size={17} />
                  </span>
                  <span style={{ flex: 1, fontSize: "var(--font-size-sm)", fontWeight: active ? 600 : 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                    {cmd.label}
                  </span>
                </button>
              );
